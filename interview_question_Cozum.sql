...
--8) Isciler tablosunda ikinci en yuksek maasi  gosteren query yazin
select *from isciler order by maas desc offset 1 limit 1;
select *from isciler where maas in (select max(maas) from isciler where maas<(select max(maas) from isciler));
